...
如果上市规则设为适用于多个网站，则网站价格的使用顺序由 [列表规则](./listing-rules.md). 这些规则允许您跨目录定义产品定价。 要查看您是否使用网站价格范围，请参阅 [目录价格范围](https://docs.magento.com/user-guide/catalog/catalog-price-scope.html){target="_blank"}.

中列出的选项 _[!UICONTROL Magento Price Source]_，_[!UICONTROL Minimum Advertised Price (Map)]_、和 _[!UICONTROL Strike Through Price (MSRP)]_包括您配置的定价属性。 定价属性为 [!DNL Commerce] “商店所有者”值设置为的目录输入类型的产品属性 `Price`. 参见 [属性输入类型](https://docs.magento.com/user-guide/stores/attributes-input-types.html){target="_blank"}.
...
1. 对象 **应用增值税(VAT)**，选择一个选项：
   ...
   >[!NOTE]
   >
   >欧盟(EU)企业需要将发票发送给商业采购员，以便客户可以汇款。 您可以自行生成这些发票并计算税款，也可以使用计税服务(如Amazon的增值税计算服务)。 Amazon建议注册 [Amazon增值税计算服务](https://sell.amazon.co.uk/learn/vat-resources?ref_=asuk_soa_rd&amp;){target="_blank"}. 如果选择其他方法，则由您负责符合VAT规定。>
   >
...
1. 对象 **[!UICONTROL Currency Conversion]**，选择一个选项。
   ...
   默认为 `Disabled`. 这些选项取决于 [!DNL Commerce] [货币](https://docs.magento.com/user-guide/configuration/general/currency-setup.html){target="_blank"} 设置。 如果没有可用选项，请设置您的货币设置。

1. 完成后，单击 **[!UICONTROL Save listing settings]**.

![挂牌价](assets/amazon-listing-price.png)
