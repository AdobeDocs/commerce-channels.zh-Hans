...
能够放弃后续规则是定价规则内的一项强大功能，可防止多个定价规则栈叠并提供意想不到的额外折扣。 要放弃后续规则，定价规则必须使用中设置的优先级。 _[!UICONTROL Priority]_部分 [定价规则常规设置](./pricing-rule-general-settings.md).

如果 **[!UICONTROL Discard Subsequent Rules]** 设置为 `Yes`，具有较低优先级（数字较高）的规则不适用于符合条件的产品。
...
在此方案中，规则#1和#2适用于符合条件的产品。 规则#3仅适用于规则#2中未包含的合格产品，因为其优先级低于示例#2和 **[!UICONTROL Discard Subsequent Rules]** 设置为 `Yes`. 因此，符合条件的销售类别产品将获得10%的折扣，以及Amazon上市价格的2美元折扣。
...
因为 [Buy Box](./buy-box-competitor-pricing.md) 价格低于原价，产品按原价列示。
...
因为 [Buy Box](./buy-box-competitor-pricing.md) 价格大于原始价格，则产品在 [Buy Box](./buy-box-competitor-pricing.md) 价格。

应用规则后的最终价格：10美元

### Buy Box价格（含底价来源） =价格和20%的价格减幅
...
因为 [Buy Box](./buy-box-competitor-pricing.md) 价格小于计算得出的值 [底价](./floor-price.md)，则计算得出的值中会列出该产品 [底价](./floor-price.md).
...
因为 [Buy Box](./buy-box-competitor-pricing.md) 价格大于计算得出的值 [底价](./floor-price.md)，该产品将列在 [Buy Box](./buy-box-competitor-pricing.md) 价格。
...
因为 [最低竞争对手价格](./lowest-competitor-pricing.md) 对于“已使用”条件为$13，则产品为$13。
...
[最高价](./optional-ceiling-price.md) 欧洲(VAT)市场：$10 x 1.25 = $12.50

当 [最高价](./optional-ceiling-price.md) 在欧洲(VAT)市场受到冲击，需要计算并增加VAT。
...
当 [最高价](./optional-ceiling-price.md) 之后，在智能定价规则之上应用标准定价规则。

应用标准定价规则后的最终价格：$13.75 + $5.00 = $18.75

### 价格调整

在本例中，最具竞争力的价格是通过查看Amazon来确定的 [竞争对手的最低价格](./lowest-competitor-pricing.md) 95%的正面反馈，最低反馈数量为1000个商户评论。
...
| 字段 | 描述 |
|--------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Price Action] | 选项：<ul><li>**[!UICONTROL Decrease By]**  — 此选项会降低您的上市价格相对于 [最低竞争对手价格](./lowest-competitor-pricing.md).</li><li>**[!UICONTROL Increase By]**  — 此选项会提高您的上市价格相对于 [最低竞争对手价格](./lowest-competitor-pricing.md).</li><li>**[!UICONTROL Match Competitor Price]**  — 此选项会根据参数更改Amazon上市价格以匹配最低价格。 在示例中，Amazon的挂牌价为25美元。</li></ul> |
| [!UICONTROL Apply] | 选项：按百分比核销/按固定金额核销 |
| [!UICONTROL Adjustment Amount] | 用于定义要应用的折扣的百分比或固定金额的数值。 <br>这些选择将获得最低价格，并将其设定为$0.01。 |
...
[最低价](./floor-price.md) 计算=最低价格来源 `$5` x下限调整金额 `5%` = $5.25
